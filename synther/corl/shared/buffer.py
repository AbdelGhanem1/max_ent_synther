...
import gin
# [MODIFIED] Replaced gym with gymnasium
...
def return_reward_range(dataset, max_episode_steps):
    ...
    # [NOTE] This assertion might fail if the dataset ends mid-episode, but usually d4rl datasets are well-formed.
...
class ReplayBuffer(ReplayBufferBase):
    ...
    def load_d4rl_dataset(self, data: Dict[str, np.ndarray]):
        ...
        # Handle rewards shape if necessary
        rew = data["rewards"]
        if len(rew.shape) == 1:
            rew = rew[..., None]
        self._rewards[:n_transitions] = self._to_tensor(rew)
        
        self._next_states[:n_transitions] = self._to_tensor(data["next_observations"])
        
        # Handle terminals shape if necessary
...
class DiffusionGenerator(ReplayBufferBase):
    def __init__(
        ...
        # [MODIFIED] Using Gymnasium
        self.env = gym.make(env_name)
        
        # [NOTE] make_inputs now uses d4rl_offline_dataset internally via env.spec.id
