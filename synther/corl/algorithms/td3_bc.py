...
# [MODIFIED] Using gymnasium and just_d4rl
...
# [ADDED] Helper to map D4RL ids to Gymnasium ids
def get_gymnasium_id(d4rl_id):
    if 'halfcheetah' in d4rl_id:
        ...
        # Fallback: try using the ID directly if it's already a gym ID
        return d4rl_id
...
def wrap_env(
    ...
    # [MODIFIED] Gymnasium v1.0+ requires 'observation_space' arg for TransformObservation
...
def eval_actor(
    ...
    for i in range(n_episodes):
        # [MODIFIED] Gymnasium reset signature
        state, info = env.reset(seed=seed + i) 
        done = False
        episode_reward = 0.0
        while not done:
            action = actor.act(state, device)
            # [MODIFIED] Gymnasium step signature
...
class TD3_BC:  # noqa
    def __init__(
            ...
            # [FIXED] Removed duplicate argument here
            discount: float = 0.99,
            tau: float = 0.005,
...
def train(config: TrainConfig):
    # [MODIFIED] Use get_gymnasium_id to fix the NameNotFound error
    gym_id = get_gymnasium_id(config.env)
    print(f"Mapped D4RL dataset '{config.env}' to Gymnasium Environment '{gym_id}'")
    env = gym.make(gym_id)

    state_dim = env.observation_space.shape[0]
    action_dim = env.action_space.shape[0]

    # [MODIFIED] Use just_d4rl to load dataset
    dataset = d4rl_offline_dataset(config.env)
    
    # Initialize Score Normalizer for later
    score_normalizer = D4RLScoreNormalizer(config.env)
    ...
    # [MODIFIED] Fixed wrapper for Gymnasium v1.0
    ...
    for t in range(int(config.max_timesteps)):
        ...
        if t % config.eval_freq == 0 or t == config.max_timesteps - 1:
            ...
            # [MODIFIED] Use D4RLScoreNormalizer instead of env.get_normalized_score
            normalized_eval_score = score_normalizer(eval_score) * 100.0
